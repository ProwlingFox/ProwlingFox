...
{#await popNextJobID() then jobid}
    <a href={"/jobs/" + jobid} class="relative bg-gradient-to-tr from-orange-600 to-orange-400 rounded-xl shadow p-4 pl-6 pr-8 card w-full flex h-60">
        <h2 class="text-2xl font-black self-end w-3/4 text-orange-100"><strong class="block text-5xl font-extralight">{$userJobsLeft}</strong> opportunities are waiting for you.</h2>
        <div class="self-center ml-auto">
            <Icon class="w-16 h-16 text-orange-100" icon="solar:circle-bottom-up-bold"/>
        </div>
...
<style lang="postcss">
    .opacity-gradient {
        /* -webkit-mask-image: -webkit-radial-gradient(10% 10%, rgba(0,0,0,1), rgba(0,0,0,0) 80%); */
